/**
 * Vietnamese Lunar Calendar (Lịch Âm) Utility
 *
 * Algorithm based on the solar-to-lunar conversion for Vietnamese calendar.
...
/**
 * Convert a solar (Gregorian) date to Vietnamese lunar calendar date.
 * @param dd Day (1-31)
 * @param mm Month (1-12)
 * @param yy Year
...
export function solarToLunar(dd: number, mm: number, yy: number, timeZone: number = 7): LunarDate {
  const k = INT((jdFromDate(dd, mm, yy) - 2415021.076998695) / 29.530588853);
 ...
 */
export function getLunarLabel(dd: number, mm: number, yy: number): string {
